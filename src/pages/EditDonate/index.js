...
export default function EditDonate() {
  ...
  useEffect(() => {
    async function getDonate() {
      ...
      setSelectedCity(response.data.donate.city);
      setSelectedUf(response.data.donate.uf);
      ...
      setInitialPosition([response.data.donate.latitude, response.data.donate.longitude]);
    }
    getDonate();
  }, [params.id]);
  ...
  async function handleSubmit(event) {
    ...
    if (response.status === 200) {
      ...
      alert.show('Erro ao editar ponto de doação', { type: 'error' })
    }
  }
