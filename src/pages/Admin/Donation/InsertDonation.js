...
export default function InsertDonation() {
  ...
  async function onSubmit(data) {
    ...
    if (number !== '' && uf !== '' && city !== '' && latitude !== '' && longitude !== '') {
      ...
      if (name !== '' && email !== '' && whatsapp !== '' && items !== '' && user_id !== '' && selectedFile !== null) {
        const response = await api.post('donate', dataPoint);

        if (response.status === 200) {
          history.push('/admin/donate');
          alert.show('Salvo com sucesso!', { type: 'success' })
        } else {
          alert.show('Erro ao cadastrar ponto de doação', { type: 'error' })
        }
      } else {
            ...
            <Grid container spacing={3}>
              <Grid item sm={12}>
                <Paper className={classes.paper}>
                  <Grid container spacing={3}>
                    <h2>Cadastrar ponto de doação</h2>
