...
export default function AdminEditDonation() {
  ...
  useEffect(() => {
    async function getDonationPoint() {
      const response = await api.get(`/donate/${params.id}`);
      setName(response.data.donate.name)
      setEmail(response.data.donate.email)
      setWhatsapp(response.data.donate.whatsapp)
      setNumber(response.data.donate.number)
      setCity(response.data.donate.city)
      setUf(response.data.donate.uf)
      setLatitude(response.data.donate.latitude)
      setLongitude(response.data.donate.longitude)
      setAddressId(response.data.donate.address_id)
      setSelectedFile(response.data.donate.image_url)
  ...
  async function onSubmit(event) {
    ...
    const response = await api.put(`donate/edit/${params.id}`, dataPoint);

    if (response.status === 200) {
      history.push('/admin/donate');
      alert.show('Salvo com sucesso!', { type: 'success' })
    } else {
      alert.show('Erro ao atualizar o ponto de doação', { type: 'error' })
    }
  }

  return (
    <div className={classes.root}>
      <MenuAdmin title={'Pontos de doações'} />
